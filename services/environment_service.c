...
uint32_t temperature_char_add(ble_es_t * p_service)
{
...
char_uuid.uuid      = BLE_UUID_TEMPERATURE_CHARACTERISTIC_UUID;
...
APP_ERROR_CHECK(err_code);
  ...
  uint32_t lightness_char_add(ble_es_t * p_service)
{
...
ble_uuid_t          char_uuid;
ble_uuid128_t       base_uuid = BLE_UUID_MY_BASE_UUID;
char_uuid.uuid      = BLE_UUID_LIGHTNESS_CHARACTERISTIC_UUID;
err_code = sd_ble_uuid_vs_add(&base_uuid, &char_uuid.type);
APP_ERROR_CHECK(err_code);
...
void enviroment_serv_init (ble_es_t *p_service)
{
    ...
    ble_uuid128_t     base_uuid = BLE_UUID_MY_BASE_UUID;
    service_uuid.uuid = BLE_UUID_ENVIRONMENT_SERVICE_UUID;
    ...
    // add characteristic 
    lightness_char_add(p_service);
    temperature_char_add(p_service);
        ...
        void lightness_char_update(ble_es_t *p_service, uint16_t *conn_handle_in,  int16_t *value)
{
       ...
       hvx_params.handle = p_service->light_char_handles.value_handle;
       hvx_params.type   = BLE_GATT_HVX_NOTIFICATION;
       hvx_params.offset = 0;
       hvx_params.p_len  = &len;
       hvx_params.p_data = (uint8_t*)value;  
